...
const mockState = {
  ...
  alerts: [] as any[],
  scheduledAlarms: {} as Record<string, number>,
  cancelledAlarms: [] as string[],
  sqliteLocalAlarms: [] as any[],
  sqliteOfflineCompletions: [] as any[],
  supabaseInvokedFunctions: [] as any[],
  supabaseDb: {
    users: [] as any[],
    groups: [] as any[],
    group_members: [] as any[],
    alarms: [] as any[],
    alarm_completions: [] as any[],
    notifications: [] as any[]
  } as any
};

const mockVibration = {
  vibrate(pattern: any, repeat?: boolean) {
  ...
  openDatabaseAsync: async () => ({
    ...
    runAsync: async (sql: string, params: any[]) => {
      if (sql.includes('INSERT OR REPLACE INTO local_alarms')) {
        ...
        if (idx !== -1) mockState.sqliteLocalAlarms[idx] = alarm;
        else mockState.sqliteLocalAlarms.push(alarm);
    ...
    getAllAsync: async (sql: string, params?: any[]) => {
      ...
      } else if (sql.includes('SELECT alarm_id FROM local_alarms')) {
        const alarmId = params![0];
        const pattern = params![1] || '';
        const regexPattern = new RegExp('^' + pattern.replace(/%/g, '.*') + '$');
        return mockState.sqliteLocalAlarms
          .filter(a => a.alarm_id === alarmId || regexPattern.test(a.alarm_id))
          .map(a => ({ alarm_id: a.alarm_id }));
      }
      return [];
    },
...
class MockQueryBuilder {
  ...
  select() { return this; }
  eq(col: string, val: any) { this.filters.push((x: any) => x[col] === val); return this; }
  neq(col: string, val: any) { this.filters.push((x: any) => x[col] !== val); return this; }
  in(col: string, vals: any[]) { this.filters.push((x: any) => vals.includes(x[col])); return this; }
  order() { return this; }
  limit() { return this; }
  async single() {
...
const mockSupabase = {
  ...
  functions: {
    ...
    mockResponses: {} as Record<string, any>,
...
let generateChallenge: (diff: 'easy' | 'medium' | 'hard') => { question: string; answer: string };
try {
  const challengePath = path.resolve(process.cwd(), 'src/app/alarm/challenge.tsx');
...
function resolveRecurringAlarmTimes(
  alarmTimeStr: string,
  ...
  for (let i = 0; i <= maxDays; i++) {
    ...
    if (checkDate <= startFrom) continue;
...
async function runVerification() {
  ...
  console.log(`  Result: Found ${future.length} future alarms. Sequence: ${future.map((f: any) => f.alarm_id).join(' -> ')}`);
  ...
  if (generateChallenge) {
    ...
    const checkMath = (expr: string, ans: string): boolean => {
  ...
  const ringPath = path.resolve(process.cwd(), 'src/app/alarm/ring.tsx');
  ...
  const splashPath = path.resolve(process.cwd(), 'src/app/index.tsx');
  const splashCode = fs.readFileSync(splashPath, 'utf8');
  const appSyncsOffline = splashCode.includes('syncOfflineCompletions');
  ...
  await insertOrUpdateAlarm({
    alarm_id: 'alarm_to_cancel',
    group_id: 'g1',
    alarm_time_utc: new Date(baseTime + 10000).toISOString(),
  ...
  const createAlarmEdgePath = path.resolve(process.cwd(), 'supabase/functions/create-alarm/index.ts');
  ...
  const uniqueRequestCodes = [(alarmGroup1.alarm_id as any).hashCode(), (alarmGroup2.alarm_id as any).hashCode()];
  ...
  const sqlitePath = path.resolve(process.cwd(), 'lib/sqlite.ts');
  const sqliteCode = fs.readFileSync(sqlitePath, 'utf8');
  const hasAppRecurrenceResolution = sqliteCode.includes('resolveRecurrence');
  const startDate = new Date('2026-06-17T06:00:00'); // Wednesday
  const resolved = resolveRecurringAlarmTimes(
    '07:30',
    [1, 3, 5], // Mon, Wed, Fri
